...
function TerrainMesh({ imageData, elevationScale = 50 }: { imageData: string; elevationScale: number }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // If transparent, set height to 0
        if (a === 0) {
          heights[i / 4] = 0;
        } else {
          // Convert RGB to grayscale (0-1 range)
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < heightData.length; i++) {
      ...
      vertices[i * 3 + 2] = height * elevationScale;
    }
...
export function Terrain3DViewer({ imageData, bounds, elevationScale = 50 }: Terrain3DProps) {
  return (
